...
// EvilWizard3 caster. Same "smart enemy" approach as ElfArcher, with a caster's toolkit:
//  * Spell choice through SpellBrain (straight bolt / homing / volley scored against the situation
//    and how many recent bolts missed) instead of a uniform random roll.
//  * Straight bolts lead the player (predictive aim); homing ones are fired straight at them.
//  * Holds the release through the player's dash (and while they block toward it), lets go right
//    after — getting hit mid-cast cancels the spell.
//  * Unless HoldPosition: keeps a distance band (backs off / walks up, never off a ledge or into a
//    wall), hops away when rushed (only if the hop lands on ground) and, with DesaparecerHit,
//    blinks away on hit or when cornered — to a spot on its own level, clear of walls, not next to
//    the player.
// Its own class (enemy-uniqueness rule); shared pieces are capabilities, not a template: Enemy's
// terrain helpers, PlayerReads, SpellBrain.
public partial class Hechicero : Enemy
{
	...
	private void TryStartCast(Metroidvania.Player.Player player)
	{
		...
		HechiceroAttack spell = _brain.Choose(Attacks, situation, RangeOf) as HechiceroAttack;
		if (spell is null)
			return;
	...
	private async Task Cast(HechiceroAttack spell)
	{
		_casting = true;
		_cancelCast = false;
		var player = PlayerReads.Find(this);
	...
	private void SpawnProjectile(HechiceroAttack spell, Metroidvania.Player.Player player, int projectilesInCast)
	{
